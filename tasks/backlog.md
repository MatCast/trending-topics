...
- [ ] The disable button for the automation schedule should make every configuration below disappear and display something saying "enable to configure it".
...
## DONE: Keyword Management & UI Refinement
Implemented a robust keyword management system with Gmail-style bulk actions (Enable, Disable, Delete). Fixed reactivity issues in UI primitives (Checkbox/Switch) by migrating to controlled `useVModel` patterns. Added duplicate keyword validation with sonar-style toast notifications. Standardized button cursors and primary brand colors across Sources, Settings, and Extraction results pages.
