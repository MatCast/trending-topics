...
4. **Context Injection**: When spawning sub-agents, **ALWAYS** include `@.agent/worker_context.md` and `@plan.md` in the instruction or via `--files` flag.
5. **Environment Setup**: In the sub-agent prompt, explicitly tell the worker which environment to use (e.g., "Use the python binary at ~/.virtualenvs/linkedin_posts/bin/python").
6. Spawn sub-agents by running `cd ./workers/<task> && gemini -y "Task description"`. Reference `cli-subagent` skill.
7. Provide specific context using `--files` or temp files.
8. Once finished, review their work using `git diff main..subagent/<task>`. Run tests in the worktree if needed.
